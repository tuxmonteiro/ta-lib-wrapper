...
public class Macd {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInFastPeriod, int optInSlowPeriod, int optInSignalPeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        try (var arena = Arena.ofConfined()) {
            ...
            int retCode = TALib.call(TALib.TA_MACD, startIdx, endIdx, inrealSeg, optInFastPeriod, optInSlowPeriod, optInSignalPeriod, outBegIdx, outNBElement, outMACDSeg, outMACDSignalSeg, outMACDHistSeg);
            if (retCode != 0) {
